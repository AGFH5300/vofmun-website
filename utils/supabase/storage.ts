...
const buildManualBucketPolicyChecklist = (bucketName: string) =>
  [
    `Supabase denied writes to the "${bucketName}" bucket due to row-level security. To allow uploads:`,
    '1. In the Supabase dashboard, open **Storage → Buckets** and select your payment proof bucket.',
    '2. Switch to the **Policies** tab and click **New policy**.',
    '3. Name it something like **Allow anon uploads**, target **Object access**, and choose the **INSERT** operation.',
    '4. Under "Allowed roles", enable both **anon** and **authenticated** (if you want logged-in users to upload).',
    '5. Set the policy rule to `true` so any request with those roles can upload proofs.',
    '6. Save the policy and retry the upload.',
    '   See docs/payment-proof-setup.md for the full policy SQL example.',
  ].join('\n')

export const getManualBucketPolicyChecklist = (bucketName: string) =>
  buildManualBucketPolicyChecklist(bucketName)
